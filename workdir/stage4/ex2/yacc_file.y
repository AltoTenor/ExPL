...
%type <no> declarations declList decl type varList varEnd
%type <no> AsgStmt InputStmt OutputStmt IfStmt WhileStmt DoWhileStmt RepeatStmt
%token <no> START END DECL ENDDECL STR INT BRKP
...
varList : varList ',' varEnd { $$ = createTree(-1, invalidType, "",connectorNode,  alloc_2($1, $3), 2, NULL); }
    | varEnd { $$ = $1; }
;
...
identifier : ID         { $$ = $1; }
    | ID '[' expr ']' {   
        ...
        $$ = createTree(-1, intType , "", arrTypeNode,  alloc_2($1, $3), 2 , g);
    }
    | ID '[' expr ']' '[' expr ']' {
        struct Gsymbol* g = Lookup($1->varname);
        if ( g == NULL ){
        ...
        $$ = createTree(-1, intType , "", arr2dTypeNode,  alloc_3($1, $3, $6), 3 , g);
    }
;
...
expr : expr '+' expr    {   $$ = createTree(-1, intType , "", addNode, alloc_2($1, $3), 2, NULL); }
    ...
    | '*' expr          {   $$ = createTree(-1, $2->type , "", derefNode, alloc_1($2), 1, NULL); }
    | NUM               {   $$ = $1; }
    | identifier        {   $$ = createTree(-1, $1->Gentry->type , "", derefNode, alloc_1($1), 1, NULL); }
    | '&' identifier    {   $$ = $2; }
;

...
program : declarations START slist END ';' {
                        $$ = createTree(-1, invalidType, "",connectorNode,  alloc_2($1, $3), 2);
                        printTree($$);
        ...
        | declarations slist {
                        $$ = createTree(-1, invalidType, "",connectorNode,  alloc_2($1, $2), 2);
                        printTree($$);
                        struct Gsymbol * temp = symbolTable;
                        while(temp){
                            printf( "%s : %d : %d : %d\n", 
...
declList: declList decl { $$ = createTree(-1, invalidType, "",connectorNode,  alloc_2($1, $2), 2); }
    | decl              { $$ = $1; }
;
...
varList : varList ',' ID { $$ = createTree(-1, invalidType, "",connectorNode,  alloc_2($1, $3), 2);}
    | ID                 { $$ = $1; }
;

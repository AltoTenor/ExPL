...
void setTypes(struct tnode* t, int type, struct Gsymbol* paramHead){
    if ( t->nodetype == connectorNode ){
        setTypes( t -> children[0], type, paramHead);
        setTypes( t -> children[1], type, paramHead);
    }
        ...
        Install( t->children[0]->varname, type, t->children[1]->val);
        t->children[0]->Gentry = Lookup(t->children[0]->varname);
        t->children[0]->type = type;
    }
    else if( t->nodetype == funcTypeGDeclNode ){
        Install( t->children[0]->varname, type, 0);
        t->children[0]->Gentry = Lookup(t->children[0]->varname);
        t->children[0]->type = type;
        setTypes( t -> children[1], type, t->children[0]->Gentry);
    }

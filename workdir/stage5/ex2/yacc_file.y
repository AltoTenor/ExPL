...
GDecl : Type GidList ';' {
    ...
    | TUPLE ID '(' ParamList ')' tupleIdList ';'{
        $$ = createTree(-1, invalidType, "", tupleNode, alloc_3($2, $4, $6), 3, NULL, NULL);
        addTupleGST($6, $4, $2->varname);
    }
;
...
IdList : IdList ',' ID {
    ...
    | IdList ',' '*' ID {
        $$ = createTree(-1, invalidType, "", connectorNode,  alloc_2($1, $4) , 2 , NULL, NULL); 
    }
    | ID { $$ = $1; }
    | '*' ID { $$ = $2; }
;
...
Param : Type ID {
    ...
    | Type '*' ID {
        $$ = createTree(-1, invalidType, "", paramNode,  alloc_2($1, $3) , 2 , NULL, NULL); 
    }
;
...
identifier : ID  { $$ = $1; }
    ...
    | ID '.' ID {
        $$ = createTree(-1, intType , "", tupleAccessNode,  alloc_2($1, $3), 2 , NULL, NULL);
    }
;
...
AsgStmt : identifier '=' expr ';' {
    ...
    | '*' expr '=' expr ';' {
        $$ = createTree(-1, invalidType, "", assignNode , alloc_2($2, $4), 2, NULL, NULL);
    }
    | '*' expr '=' STRING ';' {
        $$ = createTree(-1, invalidType, "", assignNode , alloc_2($2, $4), 2, NULL, NULL);
    }

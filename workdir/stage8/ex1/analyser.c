...
void checkFDef(struct tnode ** c, struct Gsymbol *  funcEntry){
    ...
    if ( c[2] != NULL ){
        struct Paramstruct * defParamList = fetchParamList(c[2]);
        if ( verifyParamList(defParamList, funcEntry->paramlist) == 0 ){
            printf("Parameter Mismatch in Definition and Declaration\n");
            printf("Function: %s\n", funcEntry->name);
            exit(1);
        }
    }
                            ...
                            if ( t->children[0]->Ctype ){
                                struct MemberFunclist * method 
                                    = MethodLookup( t->children[0]->children[1]->name, 
                                                    t->children[0]->Ctype, argList);
                                if ( method == NULL ){
                                    printf("Method Not Found with these Args: %s in class %s\n", 
                                        t->children[0]->children[1]->name,
                                        t->children[0]->Ctype->className);
                                    exit(1);
                                }
                                if ( method->paramlist ){
        ...
        case NODE_MEMBER:{   /* To handle - self.x or self.func()
                            ...
                            if (t->children[0]->nodetype == NODE_SELF) {
                                ...
                                struct MemberFunclist * met 
                                    = MethodLookup(t->children[1]->name, classEntry, NULL);
...
int verifyParamList(struct Paramstruct* a, struct Paramstruct* b){
    struct Paramstruct* p1 = a;
    struct Paramstruct* p2 = b;

    while (p1 && p2){
        if ( p2->type != p1->type || strcmp(p1->name, p2->name) != 0 ) return 0;
        p1 = p1->next;
        p2 = p2->next;
    }
    if ( p1 || p2 ) return 0;
...
struct Paramstruct * fetchParamList(struct tnode * t){
    if ( t == NULL ) return NULL; 
    if ( t -> nodetype == NODE_CONNECTOR ) {
        struct Paramstruct * head = NULL,* tail = NULL, *p;
        for ( int i=0; i< t->childcount; i++ ){
            p = fetchParamList(t->children[i]);
            if ( head == NULL ){
                head = p;
            }
            else {
    ...
    if ( t -> nodetype == NODE_PARAM ) {
        struct Paramstruct* entry = (struct Paramstruct*)malloc(sizeof(struct Paramstruct));
        entry->name = (char*)malloc(sizeof(char)*20);
        strcpy(entry->name, t->children[1]->name );
        entry->type = t->children[0]->type;
        return entry;
    }
}
...
struct MemberFunclist * MethodLookup(char * name, struct Classtable* CT, struct Paramstruct *p){
    ...
    while (temp){
        ...
        // No paramlist provided - just search too check if it exists  
        if ( p == NULL && strcmp(temp->methodName, name) == 0 ) return temp;
        // Paramlist provided with name of variables - check properly if they match (int, name)  
        if ( p && p->name && strcmp(temp->methodName, name) == 0 
            && verifyParamList(p, temp->paramlist) ) return temp;
        // Paramlist provided without name - check if types match
        if ( p && strcmp(temp->methodName, name) == 0 && verifyArgTypes(p, temp->paramlist) ) 
            return temp;
...
void checkMDef(struct tnode* t, struct Classtable* CTEntry){
    if ( t->nodetype == NODE_CONNECTOR ){
        ...
        struct Paramstruct * defParamList = fetchParamList(t->children[2]);
        struct MemberFunclist * method = MethodLookup(t->children[1]->name, CTEntry, defParamList);
        ...
        if ( defParamList != NULL ){
            if ( verifyParamList(defParamList, method->paramlist) == 0 ){
                printf("Parameter Mismatch in Definition and Declaration\n");
                printf("Function: %s\n", method-> methodName );
                exit(1);
            }
    ...
    while ( newMethod ){
        ...
        while ( tail ){
            if ( strcmp(tail->methodName, newMethod->methodName) == 0 
                && verifyParamList(tail->paramlist, newMethod->paramlist) ){
                redefinedFlag = 1;
